...
3.  **Dynamical Simulation**: This part implements a threshold Boolean model to simulate the network's behavior under different environmental conditions (rich media vs. starvation). The goal is to investigate whether the model can generate the phenotypic heterogeneity required for persistence by searching for multiple stable states (attractors).
...
### 1. Single cell data analysis (`Candidates` folder)
...
The YeastRGB .csv file used (load in the scripts as "C_SWAT_tab_data.csv") is available in 
the [YeastRGB](https://www.nature.com/articles/s41589-020-0576-z) library (heavy file).

Some scripts require additional data generated by others in this same folder, but all
files are upload in data/Single cell.

---

### 2. Topological Analysis (`Topology` folder)
...
### 3. Dynamical Simulations (`Dynamics` folder)
...
1.  **Set of persister gene candidates:** We use three criterias to identify the potential persister genes based on single cell data, finding within our candidates the yeast persister marker HSP12.
2.  **Single Attractor per Condition:** The deterministic Boolean model converges to a single, globally dominant attractor for each simulated environment. It does not produce the stable phenotypic heterogeneity required to explain persistence as an alternative cell state.
3.  **The Importance of Transients:** The convergence to the quiescent attractor is significantly slower and more heterogeneous than the convergence to the proliferative state. This suggests that persistence could be a long-lived transient state within a rugged dynamical landscape, rather than a fixed attractor.
...
Jaime Martínez Cazón - Academic mail: `jmartica662@alumnes.ub.edu` - Personal mail: `jaimemartinezcazon@gmail.com`  
